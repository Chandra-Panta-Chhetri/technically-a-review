...
router.get("/", async (req, res) => {
  try {
    ...
    if (
      numOfComments &&
      (Number(page) <= 0 || Number(page) > Math.ceil(numOfComments / perPage))
    ) {
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      req.flash(
        "error",
        "Cannot update comment at this time. Please try again later."
      );
      return res.redirect("/campgrounds");
    }
  }
